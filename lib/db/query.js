...
async function addNewTestAlert(testId, alertChannel) {
  ...
  try {
    console.log('ALERT CHANNEL TYPE: ', alertChannel.type);

    let result1 = await dbQuery(query1, alertChannel.alertsOnRecovery, alertChannel.alertsOnFailure);
    notificationSettingsId = result1.rows[0]['id'];
    console.log('Notifcation Settings Id: ', notificationSettingsId);

    let result2 = await dbQuery(query2, alertChannel.type, alertChannel.destination, notificationSettingsId);
    alertsId = result2.rows[0]['id'];
    console.log('Alerts Id: ', alertsId);

    let result3 = await dbQuery(query3, testId, alertsId);
    addAlertResult = result3.rowCount === 1;
    console.log('Add Alert Channel Result :', addAlertResult);
  } catch (e) {
    console.error(e);
  }
...
async function addNewTestAlerts(testId, alertChannels) {
  try {
    let result = await Promise.all(alertChannels.map((channel) => addNewTestAlert(testId, channel)));
