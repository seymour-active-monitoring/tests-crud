...
async function getSideload() {
  ...
  return {
    comparisonTypes: helpers.toCamelCase(coparisonTypesResult.rows, 'name'),
    regions: helpers.toCamelCase(regionsResult.rows, 'name'),
    httpMethods: methodQueryResult.rows,
    assertionTypes: helpers.toCamelCase(assertionTypesResult.rows, 'name'),
  };
}
